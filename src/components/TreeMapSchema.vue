      ...
      drawChart() {
        ...
        svgRoot.call(zoom().on("zoom", function () {
          ...
          svg.selectAll('line.line')
              .attr('x1', d => d.data.line[0] * transform.k + transform.x)
              .attr('y1', d => d.data.line[1] * transform.k + transform.y)
              .attr('x2', d => d.data.line[2] * transform.k + transform.x)
              .attr('y2', d => d.data.line[3] * transform.k + transform.y)

          svg.selectAll('g.label')
              .attr('transform', d => `translate(${d.data.line[2] * transform.k + transform.x}, ${d.data.line[3] * transform.k + transform.y}) rotate(${d.data.labelRotation})`)

          svg.selectAll('g.facility')
              .attr('transform', d => `translate(${d.data.line[0] * transform.k + transform.x}, ${d.data.line[1]  * transform.k + transform.y}) scale(${transform.k})`)
              // .selectAll('circle')
              // .attr('r', Math.min(30 * transform.k, 30))
        }))
        ...
        svg.selectAll('g.label')
            .data(descendants.filter(d => d.data.label))
            .join('g')
            .attr('class', 'label')
        ...
        let legend = svg.selectAll('g.legend')
          ...
          .join('g')
          .attr('class', 'legend')
          .attr('transform', (d, i) => `translate (0, ${(height-padding.bottom-(i * 30))})`)
        ...
        let facility = svg.selectAll('g.facility')
            .data(descendants.filter(d => d.data.is_facility))
            .join('g')
            .attr('class', 'facility')
            .attr('transform', d => `translate(${d.data.line[0]}, ${d.data.line[1]})`)

        facility.append('circle')
            .attr('r', 30)
            .attr('fill', 'white')
            .attr('stroke', this.data.legend.items[0].color)
            .attr('stroke-width', "2")
